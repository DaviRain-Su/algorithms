package com.mycompany.standrard;
...
public final class StdAudioStereo {
    ...
    private static AudioInputStream getAudioInputStreamFromFile(
        String filename
    ) {
        if (filename == null) {
        ...
        try {
            ...
            InputStream is1 =
                StdAudioStereo.class.getResourceAsStream(filename);
            if (is1 != null) {
            ...
            InputStream is2 =
                StdAudioStereo.class.getClassLoader()
            ...
            throw new IllegalArgumentException(
                "file of unsupported audio file format: '" + filename + "'",
                e
            );
        }
    }
...
*/
    ...
    public static void play(double sampleLeft, double sampleRight) {
        if (Double.isNaN(sampleLeft)) throw new IllegalArgumentException(
            "sampleLeft is NaN"
        );
        if (Double.isNaN(sampleRight)) throw new IllegalArgumentException(
            "sampleRight is NaN"
        );
    ...
    public static void play(double[] samplesLeft, double[] samplesRight) {
        if (samplesLeft == null) throw new IllegalArgumentException(
            "argument to play() is null"
        );
        if (samplesRight == null) throw new IllegalArgumentException(
            "argument to play() is null"
        );
        if (
            samplesLeft.length != samplesRight.length
        ) throw new IllegalArgumentException(
    ...
    public static void play(String filename) {
        ...
        if (isRecording) {
            ...
            for (double sample : samplesLeft) recordedSamplesLeft.enqueue(
                sample
            );
            for (double sample : samplesRight) recordedSamplesRight.enqueue(
                sample
            );
        }
    ...
    private static double[] readChannel(String filename, boolean channel) {
        ...
        AudioFormat toAudioFormat = new AudioFormat(
            (float) SAMPLE_RATE,
            BITS_PER_SAMPLE,
            STEREO,
            SIGNED,
        ...
        if (
            !AudioSystem.isConversionSupported(toAudioFormat, fromAudioFormat)
        ) {
            throw new IllegalArgumentException(
                "system cannot convert from " +
                fromAudioFormat +
                " to " +
                toAudioFormat
            );
        }
        AudioInputStream toAudioInputStream = AudioSystem.getAudioInputStream(
            toAudioFormat,
            fromAudioInputStream
        );
        ...
        try {
            ...
            while (
                (count = toAudioInputStream.read(bytes, 0, READ_BUFFER_SIZE)) !=
                -1
            ) {
                // little endian, stereo (perhaps, for a future version that supports stereo)
                for (int i = 0; i < count / 4; i++) {
                    double left =
                        ((short) (((bytes[4 * i + 1] & 0xFF) << 8) |
                                (bytes[4 * i + 0] & 0xFF))) /
                        ((double) MAX_16_BIT);
                    double right =
                        ((short) (((bytes[4 * i + 3] & 0xFF) << 8) |
                                (bytes[4 * i + 2] & 0xFF))) /
                        ((double) MAX_16_BIT);
    ...
    private static class BackgroundRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                ...
                int count = 0;
                while (
                    !exit && (count = ais.read(samples, 0, BUFFER_SIZE)) != -1
                ) {
                    line.write(samples, 0, count);
                }
            } catch (IOException | LineUnavailableException e) {
    ...
    public static double[] getRecordingLeft() {
        if (recordedSamplesRight != null) {
            throw new IllegalStateException(
                "getRecordingLeft() must be called after startRecording()"
            );
        }

        if (isRecording) {
            throw new IllegalStateException(
                "getRecordingLeft() must be called after stopRecording()"
            );
        }
    ...
    public static double[] getRecordingRight() {
        if (recordedSamplesRight != null) {
            throw new IllegalStateException(
                "getRecordingRight() must be called after startRecording()"
            );
        }

        if (isRecording) {
            throw new IllegalStateException(
                "getRecordingRight() must be called after stopRecording()"
            );
        }
    ...
    public static double[] getRecordingMono() {
        if (recordedSamplesRight != null) {
            throw new IllegalStateException(
                "getRecording() must be called after startRecording()"
            );
        }

        if (isRecording) {
            throw new IllegalStateException(
                "getRecording() must be called after stopRecording()"
            );
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i <= StdAudioStereo.SAMPLE_RATE; i++) {
            double sampleLeft =
                0.5 *
                Math.sin(
                    (2 * Math.PI * freq1 * i) / StdAudioStereo.SAMPLE_RATE
                );
            double sampleRight =
                0.5 *
                Math.sin(
                    (2 * Math.PI * freq2 * i) / StdAudioStereo.SAMPLE_RATE
                );
            StdAudioStereo.play(sampleLeft, sampleRight);
        }
        ...
        StdAudioStereo.play(
            StdAudioStereo.readLeft(base + "test.midi"),
            StdAudioStereo.readRight(base + "test.midi")
        );
