...
public final class Draw
    ...
    public void save(String filename) {
        ...
        if (!filename.contains(".") || suffix.length() == 0) {
            throw new IllegalArgumentException(
                "the filename '" +
                filename +
                "' has no file extension, such as .jpg or .png"
            );
        }
        ...
        try {
            ...
            throw new IllegalArgumentException(
                "the filetype '" + suffix + "' is not supported"
            );
        } catch (IOException e) {
            throw new IllegalArgumentException(
                "could not write the file + " + filename,
                e
            );
        }
    }
    ...
    public void mousePressed(MouseEvent event) {
        ...
        if (event.getButton() == MouseEvent.BUTTON1) {
            for (DrawListener listener : listeners) listener.mousePressed(
                userX(event.getX()),
                userY(event.getY())
            );
        }
    ...
    public void mouseReleased(MouseEvent event) {
        ...
        if (event.getButton() == MouseEvent.BUTTON1) {
            for (DrawListener listener : listeners) listener.mouseReleased(
                userX(event.getX()),
                userY(event.getY())
            );
        }
    ...
    public void mouseClicked(MouseEvent event) {
        if (event.getButton() == MouseEvent.BUTTON1) {
            for (DrawListener listener : listeners) listener.mouseClicked(
                userX(event.getX()),
                userY(event.getY())
            );
        }
    ...
    public void mouseDragged(MouseEvent event) {
        ...
        for (DrawListener listener : listeners) listener.mouseDragged(
            userX(event.getX()),
            userY(event.getY())
        );
    }
    ...
    public void keyTyped(KeyEvent event) {
        ...
        for (DrawListener listener : listeners) listener.keyTyped(
            event.getKeyChar()
        );
    }
    ...
    public void keyPressed(KeyEvent event) {
        ...
        // notify all listeners
        for (DrawListener listener : listeners) listener.keyPressed(
            event.getKeyCode()
        );
    }
    ...
    public void keyReleased(KeyEvent event) {
        ...
        // notify all listeners
        for (DrawListener listener : listeners) listener.keyReleased(
            event.getKeyCode()
        );
    }
    ...
    public void enableTimer(int callsPerSecond) {
        ...
        timer.schedule(
            new MyTimerTask(),
            0,
            (int) Math.round(1000.0 / callsPerSecond)
        );
    }
    ...
    private class MyTimerTask extends TimerTask {
        ...
        public void run() {
            for (DrawListener listener : listeners) listener.update();
        }
    }
