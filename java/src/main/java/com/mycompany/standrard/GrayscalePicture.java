...
public final class GrayscalePicture implements ActionListener {
    ...
    private void validateColumnIndex(int col) {
        if (col < 0 || col >= width()) throw new IndexOutOfBoundsException(
            "column index must be between 0 and " + (width() - 1) + ": " + col
        );
    }

    private void validateGrayscaleValue(int gray) {
        if (gray < 0 || gray >= 256) throw new IllegalArgumentException(
    ...
    public boolean equals(Object other) {
        ...
        for (int col = 0; col < width(); col++) for (
            ...
            this.getGrayscale(col, row) != that.getGrayscale(col, row)
        ) return false;
        return true;
    }
    ...
    public void save(File file) {
        ...
        if (!title.contains(".") || suffix.length() == 0) {
            System.out.printf(
                "Error: the filename '%s' has no file extension, such as .jpg or .png\n",
                title
            );
            return;
        }
