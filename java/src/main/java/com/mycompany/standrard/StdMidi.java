...
public class StdMidi {
    ...
    private static final String SOUNDFONT_FILENAME =
        "/usr/local/lift/Soundfonts/FluidR3_GM2-2.sf2";
    ...
    private static void validateVelocity(int val) {
        if (val < 0 || val > 127) throw new IllegalArgumentException(
            "MIDI velocity must be between 0 and 127: " + val
        );
    }

    private static void validateTempo(int val) {
        if (val <= 0) throw new IllegalArgumentException(
            "the tempo (beats per minute) must be a positive integer: " + val
        );
    }

    private static void validateNote(int val) {
        if (val < -1 || val > 127) throw new IllegalArgumentException(
            "the pitch must be between 0 and 127: " + val
        );
    }

    private static void validateDuration(double val) {
        if (val < 0.0) throw new IllegalArgumentException(
            "the number of beats must be non-negative: " + val
        );
    }

    private static void validateInstrument(int val) {
        if (val <= 0 || val > 128) throw new IllegalArgumentException(
            "the MIDI instrument number must be between 1 and 128: " + val
        );
    }
    ...
    private static void validatePercussionInstrument(int val) {
        if (
            ...
            "the percussion instrument number must be between 35 and 81: " + val
        );
    }
    ...
    public static void save(String filename) {
        ...
        try {
            ...
            if (
                ...
                throw new IllegalArgumentException(
                    "file extension for saving must be .mid"
                );
            }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < pitches1.length; i++) StdMidi.playNote(
            pitches1[i],
            beats1[i]
        );
        for (int i = 0; i < pitches2.length; i++) StdMidi.playNote(
            pitches2[i],
            beats2[i]
        );
        for (int i = 0; i < pitches3.length; i++) StdMidi.playNote(
            pitches3[i],
            beats3[i]
        );
